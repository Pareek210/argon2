...
// This function computes the set R of all the possible reference blocks for computing the next block.
// HAS to be changed.
...
void initial_hash(uint8_t *blockhash, argon2_context *context, argon2_type type) { // Change this.
    ...
    blake2b_init(&BlakeHash, ARGON2_PREHASH_DIGEST_LENGTH); // The first blake2b function to be called. Study this function and change it accordingly. 
    // All following functions need to be handled accprdingly.
...
int initialize(argon2_instance_t *instance, argon2_context *context) { // The first function to look at, called from argon2_ctx.
    ...
    /* uint8_t blockhash[ARGON2_PREHASH_SEED_LENGTH]; */
    /* Hashing all inputs */
    initial_hash(blockhash, context, instance->type); // Need to look at this. This is the first place function blake2b will be referred.
