...
int argon2_ctx(argon2_context *context, argon2_type type) { /* Checked 1 */
     ...
     * blocks
     */
    result = initialize(&instance, context); // First call to hash function -> handle this
    ...
    /* 4. Filling memory */
    result = fill_memory_blocks(&instance); // Second and most important call. Also need to handle the round function.

    if (ARGON2_OK != result) {
        return result;
    }
    /* 5. Finalization */
    finalize(context, &instance); // Final call, don't need to change that much. Chiller!
